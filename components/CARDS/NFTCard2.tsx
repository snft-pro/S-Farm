...
export default function NFTCard({ nft }: Props) {
    ...
    const { contract: businessesContract } = useContract(ANIMALS_CONTRACT_ADDRESS, "edition-drop");
    const { data: claimCondition } = useClaimConditions(businessesContract, nft.metadata.id);

    // Get the staking contract instance
    const { contract: stakingContract } = useContract(ASTAKING_CONTRACT_ADDRESS);
    ...
    const handleClaim = async () => {
        ...
        try {
            ...
            const isAproved = await businessesContract?.isApproved(
                address,
                ASTAKING_CONTRACT_ADDRESS
            );
            if(!isAproved) {
                await businessesContract?.setApprovalForAll(ASTAKING_CONTRACT_ADDRESS, true);
            }
