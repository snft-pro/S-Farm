...
export default function NFTCard({ nft }: Props) {
    ...
    const { contract: stakingContract } = useContract(MSTAKING_CONTRACT_ADDRESS);
    ...
    const handleClaim = async () => {
        ...
        try {
            ...
            const isAproved = await businessesContract?.isApproved(
                address,
                MSTAKING_CONTRACT_ADDRESS
            );
            if(!isAproved) {
                await businessesContract?.setApprovalForAll(MSTAKING_CONTRACT_ADDRESS, true);
            }
